...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            # gain[2:7] = torch.tensor(p[i].shape)[[3, 2, 3, 2, 1]]  # xyxy gain
...
class ComputeLoss_NTT:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLoss_NTT, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
    ...
    def __call__(self, p, targets, ptheta, target_theta):  # predictions, targets, model
        # print(len(p))
        device = targets.device
        ...
        target_theta = torch.unsqueeze(target_theta, 1)
        MSEloss = nn.MSELoss()
        ltheta = MSEloss(ptheta, target_theta)
        ltheta = torch.unsqueeze(ltheta, 0)
        ltheta *= self.hyp['theta']

        loss = lbox + lobj + lcls + ltheta
        return loss * bs, torch.cat((lbox, lobj, lcls, loss, ltheta)).detach()
...
class ComputeLossOTA_NTT():
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLossOTA_NTT, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
    ...
    def __call__(self, p, targets, imgs, ptheta, target_theta):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        """追加"""
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # criteria = nn.MSELoss()
                # p[0], ptheta batch_size
                # print(len(p[0]), len(targets[0]), len(ptheta))
                # print(len(targets), len(targets[0][-1]))
                # print(targets[i][:, -1])
                # print(ssss)
                # print(targets[1])
                # ltheta = criteria(ptheta, tar)
                # broadcast_theta = torch.full((1,targets[i].size()[0]),fill_value=ptheta.item(), device=device) # TODO model出力が1次元,targetと次元が合わない問題
                # broadcast_theta = torch.flatten(broadcast_theta)
                # ltheta = criteria(broadcast_theta, targets[i][:, -1]) # pred_theta, label_theta
                # ltheta = ltheta.reshape(1)
                """"""
                selected_tbox = targets[i][:, 2:6] * pre_gen_gains[i]
                selected_tbox[:, :2] -= grid
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, CIoU=True)  # iou(prediction, target)
        ...
        # print("-"*10)
        # print(ptheta.shape, len(targets[0]), len(targets[0][0]), targets[0][0][-1])
        # print(targets[0][0])
        # print(targets[0][1])
        # print(targets[0][2])
        ...
        # targets[batch_size][]
        target_theta = torch.unsqueeze(target_theta, 1)
        # print(ptheta.shape, target_theta.shape)
        # print(ptheta)
        # print(target_theta)
